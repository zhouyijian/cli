...
append 不传 `--overwrite`。本命令适合直接写入 Mermaid / PlantUML / SVG source；已编译的 OpenAPI `nodes[]` 优先使用 [`+node-create`](./lark-whiteboard-node-create.md)。两条路径都不得修改、删除或复用既有节点 ID。
...
- `mermaid` / `plantuml` / `svg` 由远端转换，可用于 initialize、普通 append 或 replace。完整源码不会自动决定写入语义。
